...
func (h *ImagePrePullJobHandler) HandleNodeActionError(
	...
	idx := -1
	for i, task := range imagePrepullJob.Status.NodeStatus {
		if task.NodeName == nodeTaskStatus.NodeName {
			task.Status = metav1.ConditionFalse
	...
	if err := status.UpdateImagePrepullJobNodeTaskStatus(ctx, h.crdcli,
		imagePrepullJob.Name, idx, nodeTaskStatus); err != nil {
