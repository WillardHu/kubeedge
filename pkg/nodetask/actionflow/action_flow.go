...
//
//	Init --> Check (--> WaitingConfirmation --> Confirm)
//	  --> BackUp --> Upgrade
//	                    └─ [If fails]-> RollBack
func initNodeUpgradeJobFlow() *Flow {
	first := &Action{Name: string(v1alpha2.NodeUpgradeJobActionInit)}
	check := &Action{Name: string(v1alpha2.NodeUpgradeJobActionCheck)}
	first.NextSuccessful = check
...
func initImagePrePullJob() *Flow {
	first := &Action{Name: string(v1alpha2.ImagePrePullJobActionInit)}
